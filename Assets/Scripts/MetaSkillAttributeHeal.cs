...
public class MetaSkillAttributeHeal : MetaSkill
{
	[WorkbookAlias("Value")]
	public int orbCount;
	...
	public override IEnumerator Execute(M3Battle battle, M3Board board, M3Player player)
	{
		...
		board.FindOrbs(orbCandidates, orbType, exclude: true, includeRecovery: false);
		...
		for (int i = 0; i < changes; i++)
		{
			...
			StartCoroutine(board.ChangeOrb(column, row, M3Orb.Recovery, delay));
		}
